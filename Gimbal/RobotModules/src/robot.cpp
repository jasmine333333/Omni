...
void Robot::updateVisionData()
{
  ...
  gimbal_ptr_->setVisionTargetDetected(vision_ptr_->isTargetDetected());
...
void Robot::runOnWorking()
{
  ...
  HW_ASSERT(scope_ptr_ != nullptr, "Scope FSM pointer is null", scope_ptr_);
  scope_ptr_->update();
  scope_ptr_->run();
};
...
void Robot::genModulesCmd()
{
  ...
  // scope
  scope_ptr_->setCtrlAngleFlag(scope_data.ctrl_angle_flag);
  scope_ptr_->setWorkingMode(scope_data.working_mode);
  if (scope_data.ctrl_angle_flag) {
    scope_ptr_->setNormPitchDelta(gimbal_data.pitch_delta);
  } else {
    scope_ptr_->setNormPitchDelta(0);
  }
  scope_ptr_->setSwitchScopeFlag(scope_data.switch_flag);
...
void Robot::setVisionCommData()
{
  ...
  if (gimbal_ptr_->getCtrlMode() == CtrlMode::Auto || feed_ptr_->getCtrlMode() == CtrlMode::Auto) {
  ...
  if (hello_world::referee::ids::GetTeamColor(robot_id) == hello_world::referee::ids::kTeamColorRed) {
    vision_ptr_->setTargetColor(Vision::TargetColor::kBlue);
  } else if (hello_world::referee::ids::GetTeamColor(robot_id) == hello_world::referee::ids::kTeamColorBlue) {
...
void Robot::setGimbalChassisCommData()
{
  ...
  // scope
  GimbalChassisComm::ScopeData::GimbalPart &scope_data = gc_comm_ptr_->scope_data().gp;
