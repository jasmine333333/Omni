...
namespace robot
{
  ...
  public:
    ...
    {
      if (state == PwrState::Dead)
        return "Dead";
      if (state == PwrState::Resurrection)
        return "Resurrection";
      if (state == PwrState::Working)
        return "Working";
      return "ErrWS";
    };
    ...
    {
      if (mode == CtrlMode::Manual)
        return ManualCtrlSrcToStr(src);
      if (mode == CtrlMode::Auto)
  ...
  private:
    ...
    uint32_t rev_counter = 0;         ///< 上一次翻转头部朝向的时间戳
